...
const TIMELINE = [
  ...
  ["provider_en_route", "On the Way", "car"],
...
export default function RequestTrackingScreen({ navigation, route }) {
  // Store the live request, refresh state, and cancellation form state.
  ...
  return (
    ...
    >
      {/* Live request header and current status. */}
      <ScreenHeader
        eyebrow="Live roadside request"
        title="Help is on the way"
        subtitle="Follow your provider's progress here."
        right={
          <StatusBadge
      ...
      />
      {/* Refresh errors and provider rejection recovery. */}
      {error ? (
        <AppCard>
          <ErrorState
            message="We couldn't update this request. Please try again."
            onRetry={loadRequest}
          />
        </AppCard>
      ) : null}
      {request.status === "provider_rejected" ? (
        <InfoBanner
          ...
          message="Choose another suitable provider to continue."
        />
      ) : null}
      {request.status === "provider_rejected" ? (
        <AppButton
      ...
      {/* Selected provider details, arrival estimate, and contact action. */}
        ...
        <View style={styles.providerTop}>
          ...
          <View style={styles.flex}>
            <Text style={styles.providerName}>
              {provider?.businessName || "Awaiting provider selection"}
            </Text>
            {provider ? (
              <Text style={styles.service}>
                {formatServiceType(provider.providerType)}
              </Text>
            ) : null}
          </View>
        </View>
      ...
      <AppCard>
        ...
        <View style={styles.timeline}>
          {TIMELINE.map(([key, label, icon], index) => {
            const complete = index <= activeIndex;
            const current = index === activeIndex;
            return (
              <View key={key} style={styles.timelineRow}>
                {index < TIMELINE.length - 1 ? (
                  <View style={[styles.line, complete && styles.activeLine]} />
                ) : null}
                <View
                  style={[
                    styles.dot,
                    complete && styles.activeDot,
                ...
                >
                  <Ionicons
                    ...
                    size={17}
                    color={complete ? colors.surface : colors.muted}
                  />
                </View>
                <View style={styles.timelineCopy}>
                  <Text
                    style={[styles.timelineText, complete && styles.activeText]}
                  >
                    {label}
                  </Text>
                  {current ? (
      ...
      {/* AI diagnosis, required service, and cost summary. */}
      <AppCard>
        ...
        <Detail
          ...
          value={formatFaultLabel(
            ...
            request.aiPrediction?.faultLabel || "Inspection required",
          )}
        />
        <Detail
        ...
        {request.estimatedCostRange ? (
          <Detail
            icon="wallet-outline"
            label="Estimated service range"
            value={`${request.estimatedCostRange.currency} ${Number(request.estimatedCostRange.min).toLocaleString()} – ${Number(request.estimatedCostRange.max).toLocaleString()}`}
            note="Actual cost may vary after inspection."
          />
        ) : null}
        {request.finalCost != null ? (
          <Detail
            ...
            label="Final cost"
            value={`LKR ${Number(request.finalCost).toLocaleString()}`}
          />
        ) : null}
      </AppCard>
      {/* Cancellation form for requests that are still active. */}
      ...
      {/* Manual refresh and request-history navigation. */}
...
// Reusable icon, label, value, and optional note row for request details.
...
const styles = StyleSheet.create({
  flex: { flex: 1 },
  providerCard: { backgroundColor: colors.surface },
  ...
  providerName: { ...typography.sectionTitle, color: colors.primaryDark },
  ...
  eta: { ...typography.pageTitle, color: colors.teal },
  ...
  timelineRow: { minHeight: 66, flexDirection: "row", gap: spacing.sm },
  line: {
    position: "absolute",
    left: 17,
    top: 34,
  ...
  activeLine: { backgroundColor: colors.teal },
  dot: {
    width: 36,
    height: 36,
    zIndex: 1,
  ...
  activeDot: { backgroundColor: colors.teal, borderColor: colors.teal },
  currentDot: { borderWidth: 4, borderColor: colors.tealLight },
  timelineCopy: { flex: 1, paddingTop: 7 },
  timelineText: { ...typography.bodyStrong, color: colors.textSecondary },
  activeText: { color: colors.textPrimary },
  current: { ...typography.caption, color: colors.teal },
  detail: {
    flexDirection: "row",
    gap: spacing.sm,
