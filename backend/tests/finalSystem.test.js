...
const {
  getServiceCostRange,
  SERVICE_COST_RANGES,
} = require("../src/config/serviceCostRanges");
...
test("[Authentication] wrong password is rejected", async (t) => {
  ...
  User.findOne = () => ({
    select: async () =>
      fakeUser("driver", { comparePassword: async () => false }),
  });
  try {
    const { res, error } = await invoke(authController.login, {
...
for (const scenario of [
  ...
  [
    "expired JWT",
    `Bearer ${jwt.sign({ id: String(new mongoose.Types.ObjectId()) }, testSecret, { expiresIn: -1 })}`,
  ],
]) {
...
const symptomCases = [
  [
    ...
    { starting_behavior: "clicking", light_condition: "dim" },
  ],
...
for (const [name, symptoms] of symptomCases) {
  test(`[Smart symptom capture] ${name} produces structured diagnostic text`, (t) => {
    ...
    const text = buildDiagnosticText({
      vehicleType: "car",
      breakdownType: name.replaceAll(" ", "_"),
      symptomCapture: capture,
    });
...
test("[AI 1 failure handling] mechanic request uses rule fallback when FastAPI is offline", async (t) => {
  const result = await aiDiagnosisService.diagnoseBreakdown(
  ...
  t.diagnostic(
    `source=${result.predictionSource}, service=${result.requiredService}`,
  );
});
...
test("[AI 2 failure handling] offline service returns no guessed repair steps", async (t) => {
  ...
  t.diagnostic(
    `status=${result.status}, repairSteps=none, mechanicFallback=true`,
  );
});
...
test("[Provider recommendation] exact, distance, filters, limit, and public response", (t) => {
  ...
  const publicResult =
    providerRecommendation.toPublicRecommendationResult(ranked);
  ...
  assert.equal(
    publicResult.providers.some((item) => item.businessName === "Unapproved"),
    false,
  );
  t.diagnostic(
    `returned=${ranked.providers.length}, first=${ranked.providers[0].businessName}, internalScoreHidden=true`,
  );
});

test("[Request tracking] only sequential provider transitions are configured", (t) => {
  assert.deepEqual(PROVIDER_STATUS_TRANSITIONS.accepted, [
    "provider_en_route",
    "on_the_way",
  ]);
  ...
  assert.equal(
    PROVIDER_STATUS_TRANSITIONS.accepted.includes("completed"),
    false,
  );
  t.diagnostic(
    "accepted -> provider_en_route -> arrived -> in_progress -> completed",
  );
});
...
test("[Cost estimation] every service range has valid LKR schema", (t) => {
  ...
  t.diagnostic(
    `validated service ranges=${Object.keys(SERVICE_COST_RANGES).length}`,
  );
});
...
test("[Mobile integration] API URL and error/loading states are presentation-safe", (t) => {
  const apiSource = fs.readFileSync(
    path.resolve(__dirname, "../../mobile/src/services/api.js"),
    "utf8",
  );
  const screens = [
    "RecommendationScreen.js",
    "RequestTrackingScreen.js",
    "SelfBreakdownAssistantScreen.js",
  ]
    .map((file) =>
      fs.readFileSync(
        path.resolve(__dirname, `../../mobile/src/screens/driver/${file}`),
        "utf8",
      ),
    )
    .join("\n");
  ...
  t.diagnostic(
    "configurableApi=true, loadingStates=true, userErrors=true, rawScores=false",
  );
});
...
test("[Security] client-facing fallbacks expose no secrets or service internals", (t) => {
  ...
  assert.doesNotMatch(
    serialized,
    /OPENAI|API_KEY|traceback|stack|python|axios/i,
  );
