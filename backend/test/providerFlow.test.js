...
test("electrical specialist ranks ahead using transparent components", () => {
  const nearby = provider({ businessName: "Nearby Electrical", specializations: ["battery_electrical_mechanic"], location: { latitude: 6.93, longitude: 79.8612 } });
  const farther = provider({ businessName: "Farther Electrical", specializations: ["electrical"], location: { latitude: 6.98, longitude: 79.8612 }, averageRating: 5 });
  const result = recommendation.rankProviders([farther, nearby], {
    requiredServiceType: "battery_electrical_mechanic", latitude: 6.9271, longitude: 79.8612, vehicleType: "car", breakdownType: "battery_issue"
  });
...
test("brake specialist is preferred over general provider", () => {
  ...
  const general = provider({ specializations: ["general_mechanic"], averageRating: 5 });
  const result = recommendation.rankProviders([general, brake], {
    requiredServiceType: "brake_mechanic", latitude: 6.9271, longitude: 79.8612, vehicleType: "car", breakdownType: "brake_problem"
  });
  assert.deepEqual(result.providers.map((item) => String(item.providerId)), [String(brake._id)]);
});
...
test("general mechanic is clearly marked as fallback when specialist is absent", () => {
  ...
  const result = recommendation.rankProviders([general], {
    requiredServiceType: "engine_mechanic", latitude: 6.9271, longitude: 79.8612, vehicleType: "car", breakdownType: "engine_overheating"
  });
...
test("outside-radius, unapproved, inactive, and unavailable providers are excluded", () => {
  const candidates = [
    provider({ specializations: ["engine_mechanic"], location: { latitude: 7.5, longitude: 79.8612 }, serviceRadiusKm: 2 }),
    provider({ specializations: ["engine_mechanic"], isApproved: false }),
    provider({ specializations: ["engine_mechanic"], isActive: false }),
    provider({ specializations: ["engine_mechanic"], availabilityStatus: "busy" })
  ];
...
test("rejected provider is not recommended again", () => {
  ...
  const result = recommendation.rankProviders([rejected], {
    requiredServiceType: "brake_mechanic", latitude: 6.9271, longitude: 79.8612, vehicleType: "car", breakdownType: "brake_problem", rejectedProviderIds: [rejected._id]
  });
  assert.equal(result.providers.length, 0);
});
...
test("no-provider response is non-failing and offers retry/towing", async () => {
  ...
  } finally { ProviderProfile.find = originalFind; }
});
...
test("recommendation endpoint rejects a driver who does not own the request", async () => {
  ...
  BreakdownRequest.findById = async () => request({ driverId: new mongoose.Types.ObjectId() });
  try {
    const { res, error } = await invoke(providerController.getProviderRecommendations, {
      query: { requestId: String(new mongoose.Types.ObjectId()) }, user: { _id: driverId }
    });
    assert.equal(res.statusCode, 403);
    assert.match(error.message, /Only the driver/);
  } finally { BreakdownRequest.findById = originalFind; }
});
...
test("AI 2 escalation uses the owned session recommended service", async () => {
  ...
  aiDiagnosisService.diagnoseBreakdown = async () => ({
    predictedFault: "brake_system_fault", faultLabel: "Brake System Problem",
    requiredService: "general_mechanic", confidence: 0.9, confidenceLevel: "high",
    predictionMargin: 0.7, isAmbiguous: false, needsMoreInformation: false,
    topPredictions: [], predictionSource: "ai_model"
  });
  BreakdownRequest.create = async (payload) => { saved = payload; return payload; };
  try {
    const { res, error } = await invoke(controller.createBreakdownRequest, {
      user: { _id: driverId },
      body: {
        troubleshootingSessionId: String(troubleshootingId), vehicleType: "car",
        breakdownType: "brake_problem", urgencyLevel: "high",
        problemDescription: "Brake warning", location: { latitude: 6.9271, longitude: 79.8612 }
      }
    });
    assert.equal(error, undefined);
    assert.equal(res.statusCode, 201);
    assert.equal(saved.requiredServiceType, "brake_mechanic");
    assert.equal(String(saved.troubleshootingSessionId), String(troubleshootingId));
  } finally {
...
test("driver selection creates a provider_requested assignment", async () => {
  ...
  try {
    const { res, error } = await invoke(controller.selectProvider, {
      params: { id: String(item._id) }, user: { _id: driverId }, body: { providerId: String(chosen._id) }
    });
    assert.equal(error, undefined);
...
test("wrong provider cannot accept an assigned request", async () => {
  ...
  const item = request({ status: "provider_requested", selectedProviderId: selectedId });
  const originalRequestFind = BreakdownRequest.findById;
  const originalFindOne = ProviderProfile.findOne;
  BreakdownRequest.findById = async () => item;
  ProviderProfile.findOne = async () => provider({ _id: new mongoose.Types.ObjectId() });
  try {
    const { res, error } = await invoke(controller.acceptBreakdownRequest, {
      params: { id: String(item._id) }, user: { _id: providerUserId }, body: { estimatedArrivalMinutes: 20 }
    });
    assert.equal(res.statusCode, 403);
...
test("selected provider accepts and progresses through the full tracking lifecycle", async () => {
  const selected = provider({ completedJobs: 0, async save() { this.saved = true; } });
  const item = request({ status: "provider_requested", selectedProviderId: selected._id });
  const originalRequestFind = BreakdownRequest.findById;
  const originalFindOne = ProviderProfile.findOne;
  BreakdownRequest.findById = () => queryFor(item);
...
test("provider rejection clears assignment and records exclusion", async () => {
  ...
  try {
    const { res, error } = await invoke(controller.rejectBreakdownRequest, {
      params: { id: String(item._id) }, user: { _id: providerUserId }, body: { reason: "Busy" }
    });
    assert.equal(error, undefined);
...
test("invalid provider status jump is rejected", async () => {
  ...
  const item = request({ status: "accepted", selectedProviderId: selected._id });
  const originalRequestFind = BreakdownRequest.findById;
  const originalFindOne = ProviderProfile.findOne;
  BreakdownRequest.findById = async () => item;
  ProviderProfile.findOne = async () => selected;
  try {
    const { res, error } = await invoke(controller.updateBreakdownRequestStatus, {
      params: { id: String(item._id) }, user: { _id: providerUserId }, body: { status: "completed" }
    });
...
test("completed request review updates provider average once", async () => {
  const selected = provider({ averageRating: 4, totalReviews: 2, async save() { this.saved = true; } });
  const item = request({ status: "completed", selectedProviderId: selected._id, review: null });
  const originalRequestFind = BreakdownRequest.findById;
  ...
  try {
    const { res, error } = await invoke(controller.reviewBreakdownRequest, {
      params: { id: String(item._id) }, user: { _id: driverId }, body: { rating: 5, comment: "Helpful" }
    });
    assert.equal(error, undefined);
    assert.equal(res.statusCode, 201);
